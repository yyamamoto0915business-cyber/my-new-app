...
export function EventImageInput({ url, onChangeUrl, alt, compact = false }: Props) {
  ...
  const urlDetails = (
    <details className="group rounded-[10px] border border-[#e8e6e0] bg-white">
      <summary className="flex cursor-pointer list-none items-center justify-between gap-2 px-3.5 py-2.5 text-[13px] font-medium text-[#4a4844] [&::-webkit-details-marker]:hidden">
        ...
        <span className="text-[11px] font-normal text-[#888]">任意</span>
      </summary>
  ...
  if (compact) {
    return (
      <div className="space-y-3 rounded-[10px] border border-[#e8e6e0] bg-[#fafaf8] p-3.5">
        <div className="overflow-hidden rounded-[10px] border border-[#e0ddd6] bg-white shadow-sm">
          <EventThumbnail
            imageUrl={url.trim() || null}
            alt={alt || "イベント画像"}
            rounded="lg"
            placeholderSize="sm"
            className="!aspect-video max-h-[7.5rem]"
          />
        </div>

        <p className="text-[13px] leading-relaxed text-[#4a4844]">{hint}</p>
