...
function IconSvg({ icon }: { icon: IconType }) {
  const className = "h-5 w-5 shrink-0 text-[var(--foreground-muted)]";
  if (icon === "messages") {
    return (
...
export function ProfileMenuLink({ href, icon, title, subtitle, badge }: Props) {
  return (
    ...
    >
      ...
      <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 shrink-0 text-[var(--foreground-muted)]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
