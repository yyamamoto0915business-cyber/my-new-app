...
export type EventPrimaryActionsProps = {
  eventId: string;
...
const interestedSubBtnClass =
  "flex min-h-[var(--mg-touch-min)] w-full items-center justify-center rounded-[var(--mg-radius)] border border-[var(--mg-line)] bg-white px-3 text-sm font-medium text-[var(--mg-muted)] transition active:bg-zinc-50";
    ...
    <section id="event-cta" className="space-y-4">
      <div className="space-y-4 sm:hidden">
        {participationMode === "required" && (
          <>
            {applied ? (
              <div className="flex min-h-[52px] items-center justify-center rounded-[var(--mg-radius)] border border-emerald-200 bg-emerald-50/80 px-4 py-3 text-sm font-semibold text-emerald-900">
                申込済みです
              </div>
            ) : (
              <button
        ...
        {participationMode === "optional" && (
          <>
            <p className="text-xs leading-relaxed text-[var(--mg-muted)]">
              「参加予定」はマイページで一覧できます。
            </p>
            <button
              type="button"
        ...
        {participationMode === "none" && (
          <>
            <p className="text-xs leading-relaxed text-[var(--mg-muted)]">
              主な操作は<span className="font-semibold text-[var(--mg-ink)]">参加予定にする</span>
              です。あとで見るは補助的に使えます。
            </p>
            <button
              type="button"
        ...
        {participationMode === "none" && (
          <button
            ...
            disabled={reactionLoadingType !== null}
            className={cn(
              interestedSubBtnClass,
              myReaction === "interested" &&
                "border-[var(--accent)]/40 bg-[var(--mg-accent-soft)] text-[var(--accent)]"
            )}
          >
          ...
          <>
            {applied ? (
              ...
              <button
                ...
                onClick={handleJoin}
                disabled={joining}
                className="flex min-h-[var(--mg-touch-min)] w-full items-center justify-center rounded-xl bg-[var(--accent)] py-3.5 text-[15px] font-bold text-white transition-opacity hover:opacity-90 disabled:opacity-60"
              >
                {joining
                  ? "処理中..."
                  : !user && !authDisabled
          ...
          <>
            ...
            <div className="flex gap-2">
              <button
                ...
                onClick={() => handleReaction("planned")}
                disabled={reactionLoadingType !== null}
                className={`flex min-h-[var(--mg-touch-min)] flex-1 items-center justify-center rounded-xl py-3.5 text-[15px] font-bold transition ${
                  myReaction === "planned"
          ...
          <>
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                onClick={() => handleReaction("interested")}
                disabled={reactionLoadingType !== null}
                className={`flex min-h-[var(--mg-touch-min)] flex-1 items-center justify-center rounded-xl py-3.5 text-[15px] font-semibold transition ${
                  myReaction === "interested"
