...
import { Link2, ChevronRight, CircleCheck, MessageCircle } from "lucide-react";
...
export function EventDetailPcSidebar({
  ...
  const priceLine =
    price === 0 ? "無料" : `¥${Number(price).toLocaleString("ja-JP")}`;
  const priceSuffix = priceNote ? `（${priceNote}）` : "";
  ...
  return (
    <aside className="flex flex-col gap-2.5">
      ...
      <div className="rounded-2xl border border-[#e8edd8] bg-white p-4">
        ...
        {isAvailable ? (
          <button
            ...
            className="mt-4 flex h-10 w-full items-center justify-center gap-2 rounded-xl bg-[#348b38] text-[13px] font-bold text-white transition hover:bg-[#2d7a32] disabled:opacity-50"
          >
      ...
      <div className="rounded-2xl border border-[#e8edd8] bg-white p-4">
        ...
        <div className="flex flex-col gap-2.5">
          <div className="flex gap-3.5">
            ...
            <span className="text-[13px] leading-[1.55] text-[#2c3c2a]">{dateLine}</span>
          </div>
          <div className="flex gap-3.5">
            <span className="min-w-[34px] shrink-0 pt-[1px] text-[12px] text-[#8a9e80]">
              場所
            </span>
            <span className="text-[13px] leading-[1.55] text-[#2c3c2a]">
              {location}
              {address ? (
        ...
        {isAvailable ? (
          <div className="mt-3 inline-flex items-center gap-1.5 rounded-full bg-[#eef5ef] px-2.5 py-1 text-[11.5px] font-medium text-[#348b38]">
      ...
      {/* シェアする */}
      <div>
        <p className="mb-2 text-[12.5px] font-semibold text-[#1a2818]">シェアする</p>
        <div className="flex items-center gap-2">
          ...
          <a
            ...
            className="flex h-9 w-9 items-center justify-center rounded-full bg-[#1877f2] transition hover:opacity-80"
          >
            <svg viewBox="0 0 24 24" className="h-[19px] w-[19px]" fill="#fff" aria-hidden>
          ...
          <a
            ...
            rel="noopener noreferrer"
            aria-label="LINEでシェア"
            className="flex h-9 w-9 items-center justify-center rounded-full bg-[#06c755] transition hover:opacity-80"
          >
            <svg viewBox="0 0 40 40" className="h-[19px] w-[19px]" fill="#fff" aria-hidden>
          ...
          <button
            ...
            className="flex h-9 w-9 items-center justify-center rounded-full border border-[#e0e8d4] bg-[#f0f4ec] transition hover:opacity-80"
          >
            <Link2 className="h-[17px] w-[17px] stroke-[2] text-[#526448]" aria-hidden />
          </button>
          {copyDone ? (
            <span className="text-[12px] font-medium text-[#348b38]">コピーしました</span>
          ) : null}
        </div>
      </div>
    </aside>
