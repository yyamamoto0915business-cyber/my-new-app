...
const bodySchema = z.object({
  status: z.enum(["draft", "public", "closed"]),
});
...
export async function POST(
  ...
  if (!id) {
    return NextResponse.json(err("VALIDATION_ERROR", "recruitment ID が不正です"), {
      status: 400,
    });
  }
  ...
  if (!before) {
    return NextResponse.json(err("NOT_FOUND", "募集が見つかりません"), {
      status: 404,
    });
  }
  ...
  try {
    await supabase.from("admin_logs").insert({
      ...
      action_type: "update_recruitment_status",
      before_value: { status: before.status },
      after_value: { status: parsed.data.status },
      reason: `recruitment_id=${id}`,
      metadata: { recruitment_id: id },
    });
  } catch {
    // ignore log failure
  }
